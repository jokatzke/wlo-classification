def main():
    ...
    X_input_ids, X_attn_masks = generate_training_data(
        df, X_input_ids, X_attn_masks, tokenizer
    )
    ...
    BATCH_SIZE = 64
    dataset = ds.batch(BATCH_SIZE, drop_remainder=True).prefetch(
    ...
    attn_masks = tf.keras.layers.Input(
        shape=(256,), name="attention_mask", dtype="int32"
    )

    bert_embds = model.bert(input_ids, attention_mask=attn_masks)[1]
    intermediate_layer = tf.keras.layers.Dense(
        512, activation="relu", name="intermediate_layer"
    )(bert_embds)
    output_layer = tf.keras.layers.Dense(
        len(df["discipline"].value_counts()), activation="softmax", name="output_layer"
    )(intermediate_layer)
